...
Having this attribute a \ssubject{} can give the four attributes above to other \ssubjects{} in the system.

\subsection{Requests and decisions}
In a computer system the \ssubjects{} are not directly accessing objects in the system, it is processes in the system that act on behalf of the \ssubject{}.
In the following a user requesting access to a file will be written as a \ssubjects{} requesting access and the response to this request a decision.
...
So that for a \ssubject{} $S_i$ requesting read access $\underline{r}$ for \ssubject{} $S_k$ for object $O_j$, $S_i$ must have both $\underline{r}$ as well as control access $\underline{c}$ for $O_j$ itself.
...
\paragraph{Decisions}
Any request made will be met with a response of values $\underline{yes}$, $\underline{no}$, $\underline{?}$, or $\underline{error}$.
