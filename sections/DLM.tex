...
When doing assignment for instance if we have $a = 4$ and we want to assign variable $b$ to $a$.
First we read the \xvalue{} from $a$, this means the \xvalue{}, $4$, has the same label as $a$.
Now we assign the \xvalue{}, $4$, to $b$, this means the \xvalue{} gets the label of the variable $b$ and its own label, the label it got from $a$, is forgotten.
This process is what we call relabeling and more specific this kind of relabeling is called restriction.
