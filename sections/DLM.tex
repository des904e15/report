...
\mikkelin{Jeg synes $<space>$ ser lidt underligt ud.
Så er jeg mere fan af $\policy{p}{}$.
Det kan måske se lidt underligt når det står alene, men så snart det er i en label giver det fint mening: $\{\policy{p}{}\}$.}
...
\mikkelin{Jeg har skrevet eksemplet lidt om og smidt det i et example environment.
Jeg synes ikke jeg kan finde noget sted i kilden hvor de siger at de to labels er ``$=$''.}
\end{example}

When a label interprets its privacy policies it understands it as a set of flows.
\mikkel{Jeg synes vi skal omskrive det her til hvordan man skal forstå labels/flows, fremfor hvordan en label selv fortolker sine policies.
Jeg er ikke fan af at skrive \emph{hvad en label ``gør''}.}
...
\mikkelin[mere forklaring]{Jeg synes det refererer tilbage til at vi mangler en mere forklarende introduktion der kunne motivere for labels - så burde behovet for at beskrive flow følge naturligt.

Men jeg er enig i at der mangler en beskrivelse af hvad flow mængden er.}
...
\mikkelin{Jeg synes vi skal flytte de har notationer til starten og så bruge dem i hele teksten.
Det er umiddelbart et lidt sent tidspunkt at introducere dem.}
...
\begin{definition}
  ...
  \mikkelin{Der mangler forklaring af $\sqcup$ - det er ikke umiddelbart nemt at sammenholde teksten med definitionen.}
\end{definition}
...
\mikkel{Jeg har ikke forstået hvad der står i den her paragraf...}
...
\mikkel{Hvad er præcedens for $sqsubseteq$ og $\sqcup$?}
...
\end{center}
\mikkelin{Så vidt jeg kan se er det lige blevet gentaget tre gange hvad $L_A$ er. Eller har jeg misforstået noget?}
...
\mikkel{Acts for burde blive forklaret heromkring sådan at det passer med det emne det relaterer sig til.}
\begin{itemize}
